...
import static test.bcm.retrier.common.serialization.CustomSerdes.ListSerde.SerializationStrategy;
...
public class ListSerializer<Inner> implements Serializer<List<Inner>> {
    ...
    public ListSerializer(Serializer<Inner> inner) {
        ...
        this.inner = inner;
        this.serStrategy = FIXED_LENGTH_SERIALIZERS.contains(inner.getClass()) ? SerializationStrategy.CONSTANT_SIZE :
            SerializationStrategy.VARIABLE_SIZE;
    }
    ...
    public void configure(Map<String, ?> configs, boolean isKey) {
        ...
        final String innerSerdePropertyName = isKey ? CustomCommonClientConfigs.DEFAULT_LIST_KEY_SERDE_INNER_CLASS :
        ...
        try {
            ...
            serStrategy = FIXED_LENGTH_SERIALIZERS.contains(inner.getClass()) ? SerializationStrategy.CONSTANT_SIZE :
                SerializationStrategy.VARIABLE_SIZE;
            ...
            for (Inner entry : data) {
                if (entry == null) {
                    if (serStrategy == SerializationStrategy.VARIABLE_SIZE) {
                        out.writeInt(CustomSerdes.ListSerde.NULL_ENTRY_VALUE);
                    }
                } else {
