...
public class RedirectProcess implements Processor<String, String> {
     ...
     * Use kafka header, origin-event-key, to search and obtain it's origin key
    ...
    private void processTombstoneRecord(String key, KeyValueStore<String, List<String>> store) {
        ...
        log.info("--- Event with id: {} was processed successfully in Retrier!. Removing eventId from the in memory store!", originEventKey);
        eventIds.remove(key);
        if (eventIds.isEmpty()) {
            ...
            log.info("--- Sending updated event id list: {} to the store: {}", eventIds, ExampleConstant.REDIRECT_PERSON_IN_MEMORY_EVENT_STORE);
